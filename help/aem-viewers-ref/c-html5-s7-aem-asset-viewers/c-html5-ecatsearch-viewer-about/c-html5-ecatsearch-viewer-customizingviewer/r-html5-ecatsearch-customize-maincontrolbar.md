...
>[!NOTE]
>
>Este botão oferece suporte aos seletores de atributos `state` e `selected`, que podem ser usados para aplicar capas diferentes a estados de botão diferentes. Especificamente, `state="selected"` corresponde ao estado inicial do botão de rolagem quando é possível rolar o conteúdo da barra de controle para a esquerda. O `state="default"` corresponde ao estado quando o conteúdo é rolado totalmente para a esquerda, e o botão de rolagem sugere retorná-lo ao estado inicial.
