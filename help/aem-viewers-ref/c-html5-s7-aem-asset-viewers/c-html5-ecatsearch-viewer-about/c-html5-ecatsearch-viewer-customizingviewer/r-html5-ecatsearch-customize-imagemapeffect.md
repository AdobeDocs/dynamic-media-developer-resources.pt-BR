...
description: Dependendo do valor do parâmetro de modo, o visualizador exibe ícones de mapa de imagem sobre a exibição principal em lugares onde os mapas são originalmente criados no Dynamic Media Classic ou renderiza regiões exatas que correspondem à forma dos mapas de imagem originais.
...
Dependendo do valor do parâmetro de modo, o visualizador exibe ícones de mapa de imagem sobre a exibição principal em lugares onde os mapas são originalmente criados no Dynamic Media Classic ou renderiza regiões exatas que correspondem à forma dos mapas de imagem originais.
...
>[!NOTE]
>
>O `s7mapoverlay` A classe CSS usada para criar ícones de mapa de imagem no passado agora está obsoleta; use `s7icon` em vez disso.
...
>[!NOTE]
>
>O ícone do mapa de imagem é compatível com o `state` seletor de atributos, que pode ser usado para aplicar capas diferentes aos estados de ícones de `default` e `active`.
