...
# Personalização do visualizador básico de zoom{#customizing-basic-zoom-viewer}
...
Ao criar CSS personalizado, lembre-se de que o visualizador atribui a classe `.s7basiczoomviewer` ao elemento DOM do contêiner. Se você estiver usando um arquivo CSS externo passado com o comando `style=`, use a classe `.s7basiczoomviewer` como classe pai no seletor descendente para suas regras CSS. Se você estiver fazendo estilos incorporados na página da Web, qualifique também esse seletor com uma ID do elemento DOM do contêiner da seguinte maneira:
...
Usar marcadores CSS é a maneira mais flexível de criar CSS para design responsivo, pois permite direcionar não apenas o tamanho da tela do dispositivo, mas o tamanho real do visualizador, o que é útil para layouts projetados responsivos.
