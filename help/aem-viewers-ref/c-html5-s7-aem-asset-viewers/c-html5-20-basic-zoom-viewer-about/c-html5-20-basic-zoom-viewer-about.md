...
O Visualizador de zoom básico representa um arquivo JavaScript e um conjunto de arquivos auxiliares baixados pelo visualizador no tempo de execução. Basicamente, é uma inclusão única do JavaScript com todos os componentes do SDK do visualizador usados por esse visualizador, ativos e CSS específicos.

Você pode usar o Visualizador de zoom básico no modo pop-up usando uma página de HTML pronta para produção fornecida com Visualizadores IS ou no modo incorporado, onde ela é integrada à página da Web de destino usando a API documentada.
...
O modo pop-up é o mais comum para dispositivos móveis. A página da Web carrega o visualizador usando o `window.open()` Chamada de JavaScript, configurada corretamente `A` elemento HTML ou qualquer outro método adequado.

É recomendável usar uma página de HTML pronta para uso para o modo de operação pop-up. Nesse caso, é chamado de [!DNL BasicZoomViewer.html] e estiver localizado na [!DNL html5/] subpasta da implantação padrão do IS-Viewers:
...
1. Adicionar o arquivo JavaScript do visualizador à página da Web.
   ...
   A criação de um visualizador exige a adição de uma tag de script no cabeçalho de HTML. Antes de usar a API do visualizador, inclua [!DNL BasicZoomViewer.js]. A variável [!DNL BasicZoomViewer.js] O arquivo está localizado sob o [!DNL html5/js/] subpasta da implantação padrão do IS-Viewers:
...
1. Definir o tamanho do visualizador
   ...
   Você pode definir `stagesize` modificador no registro de predefinição do visualizador no Dynamic Media Classic. Ou você pode passá-lo explicitamente com o código de inicialização do visualizador com `params` ou, como uma chamada de API conforme descrito na seção Referência de comandos, da seguinte maneira:
