...
Ao criar CSS personalizado, lembre-se de que o visualizador atribui `.s7spinviewer` à classe do elemento DOM do contêiner. Se você estiver usando um arquivo CSS externo passado com `style=` , use `.s7spinviewer` classe como classe pai no seletor descendente de suas regras CSS. Se você estiver fazendo estilos incorporados na página da Web, também qualifique esse seletor com uma ID do elemento DOM do contêiner da seguinte maneira:
