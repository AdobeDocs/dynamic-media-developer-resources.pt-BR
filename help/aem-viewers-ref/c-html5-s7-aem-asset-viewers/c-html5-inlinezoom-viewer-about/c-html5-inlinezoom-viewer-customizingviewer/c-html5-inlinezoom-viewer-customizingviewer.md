...
seo-title: Personalização do Visualizador de zoom incorporado
solution: Experience Manager
title: Personalização do Visualizador de zoom incorporado
topic: Dynamic media
...
# Personalização do Visualizador de zoom incorporado{#customizing-inline-zoom-viewer}
...
Além disso, é importante preservar a seguinte declaração de CSS do visualizador padrão CSS, pois é necessária para a funcionalidade de zoom em linha:
...
Ao criar CSS personalizado, lembre-se de que o visualizador atribui a `.s7flyoutviewer` classe ao elemento DOM do container. Se você estiver usando um arquivo CSS externo transmitido com `style=` comando, use a classe `.s7flyoutviewer` como classe pai no seletor descendente para suas regras CSS. Se você estiver fazendo estilos incorporados na página da Web, qualifice adicionalmente este seletor com uma ID do elemento DOM do container da seguinte maneira:
...
Com uma abordagem clássica ao estilo, o CSS teria uma referência separada ao arquivo de imagem individual no servidor para cada estado do elemento da interface do usuário. A seguir está um exemplo de CSS para estilizar um botão de rolagem:
