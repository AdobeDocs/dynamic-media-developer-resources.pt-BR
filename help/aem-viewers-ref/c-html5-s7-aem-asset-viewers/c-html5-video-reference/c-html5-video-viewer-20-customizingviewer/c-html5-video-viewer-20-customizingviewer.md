...
seo-title: Personalização do visualizador de vídeo
...
Ao criar CSS personalizado, lembre-se de que o visualizador atribui a `.s7videoviewer` classe ao elemento DOM do container. Se você usar um arquivo CSS externo transmitido com o `style=` `.s7videoviewer` comando, use a classe como classe pai no seletor descendente para suas regras CSS. Se você incorporar estilos na página da Web, qualifice adicionalmente este seletor com uma ID do elemento DOM do container da seguinte maneira:
