...
Ao criar CSS personalizado, lembre-se de que o visualizador atribui a classe `.s7videoviewer` ao elemento DOM do contêiner. Se você usar um arquivo CSS externo passado com o comando `style=`, use a classe `.s7videoviewer` como a classe pai no seletor descendente para suas regras CSS. Se você incorporar estilos na página da Web, qualifique também esse seletor com uma ID do elemento DOM do contêiner da seguinte maneira:
