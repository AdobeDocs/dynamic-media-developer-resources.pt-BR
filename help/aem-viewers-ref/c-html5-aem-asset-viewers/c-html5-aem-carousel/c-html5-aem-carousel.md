...
## Uso do Carousel Viewer {#section-e6c68406ecdc4de781df182bbd8088b4}
...
## Como incorporar o Visualizador do carrossel {#section-6bb5d3c502544ad18a58eafe12a13435}
...
É recomendável usar uma página HTML predefinida para o modo de operação de pop-up. Nesse caso, ele é chamado `CarouselViewer.html` e está localizado na `html5/` subpasta de sua implantação padrão do IS-Viewers:
...
1. Adicionar o arquivo JavaScript do visualizador à sua página da Web.
   ...
   A criação de um visualizador requer a adição de uma tag de script no cabeçalho HTML. Antes de poder usar a API do visualizador, verifique se você incluiu [!DNL CarouselViewer.js]. O [!DNL CarouselViewer.js] arquivo está localizado na [!DNL html5/js/] subpasta da implantação padrão do IS-Viewers:
...
1. Definição do tamanho do visualizador
   ...
   Você pode definir o tamanho estático para o visualizador declarando-o para a classe CSS de nível `.s7carouselviewer` superior em unidades absolutas ou usando o `stagesize` modificador.
   ...
   Consulte [Personalização do Visualizador](../../c-html5-aem-asset-viewers/c-html5-aem-carousel/c-html5-aem-carousel-customizingviewer/c-html5-aem-carousel-customizingviewer.md#concept-73a8546acdb444a387c49969ceca57d0) do carrossel para obter mais informações sobre como estilizar o visualizador com CSS.
   ...
   Você pode passar explicitamente o `stagesize` modificador com o código de inicialização do visualizador com `params` a coleção ou como uma chamada de API conforme descrito na seção Referência de comando, desta forma:
...
Todas as etapas acima são as mesmas que com a incorporação de tamanho fixo. Adicione o container `DIV` ao existente `"holder"``DIV`. O código a seguir é um exemplo completo. Observe como o tamanho do visualizador muda quando o navegador é redimensionado e como a proporção do visualizador corresponde ao ativo.
