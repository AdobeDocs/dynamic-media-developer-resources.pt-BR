...
Ao criar um CSS personalizado, lembre-se de que o visualizador atribui a `.s7interactivevideoviewer` classe ao elemento DOM do container. Se você estiver usando um arquivo CSS externo transmitido com `style=` comando, use a classe `.s7interactivevideoviewer` como classe pai no seletor descendente para suas regras CSS. Se você estiver fazendo estilos incorporados na página da Web, qualifice adicionalmente este seletor com uma ID do elemento DOM do container da seguinte maneira:
