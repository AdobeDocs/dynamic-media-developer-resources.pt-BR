...
* `\charscaley` dimensiona de forma diferente ao usar `text=` mas não afeta a altura da linha.

* Se a última linha de texto não couber, a linha inteira será solta em vez de aparecer como corte.
* `\slmult` e `\sl` se comporte de forma diferente do MS Word e `text=`, elas simplesmente entrarão em vigor para os parágrafos atuais e subsequentes.

* `\sb` aplica-se ao primeiro parágrafo para MS Word e `text=`, ADOBE INDESIGN e [!DNL Photoshop] não faça isso.
...
* Escapar o caractere sublinhado ( `\_`) em uma sequência RTF não funciona com todas as fontes que usam `textPs=`
...
* `posN=`, `anchor=`, `anchorN=`, `origin=`, e `originN=` agora retorna um erro de análise se qualquer valor do modificador for maior que 2147483648.
...
* A DefaultImage agora aplica atributos de miniatura ao usar `req=tmb`.
...
* Cores de primeiro plano no RTF ( `\cf`) não são compatíveis com texto PhotoFont.
...
* A pesquisa baseada em arquivo não tem suporte quando a tradução de localidade está habilitada na empresa [!DNL ini] arquivo.
* O Servidor de imagens não grava dados não fechados [!DNL Photoshop] caminhos corretamente.
...
* A conversão de cores pode não estar correta quando `bgColor=` não corresponde ao espaço de cor base que envolve perfis de cor.
...
`ImageConvert`às vezes, o trava com uma falha de segmentação quando interrompido com um `control-c`.
