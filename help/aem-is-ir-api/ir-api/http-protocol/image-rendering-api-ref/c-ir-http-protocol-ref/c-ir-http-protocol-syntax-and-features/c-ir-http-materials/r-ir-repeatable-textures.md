...
Texturas repetidas podem ser aplicadas a objetos planos, de linha flutuante, de rascunho, de plano, de parede e de gabinete. Quando aplicado a um objeto sem textura, o objeto é pintado com `color=` (ou `bgc=` se não `color=` for especificado).

Um material é considerado uma textura se incluir um `src=` atributo que especifica uma imagem e se ocorrer em um MSS diferente de uma borda de decalque ou de parede.

Durante a renderização, a textura é alinhada com o objeto ao corresponder o `anchor=` ponto do material de textura ao ponto de origem de textura do objeto (como criado na vinheta).
