...
Texturas repetidas podem ser aplicadas a objetos planos, flowline, rascunho, plano, parede e cabinet. Quando aplicado a um objeto não texturizável, o objeto é pintado com `color=` ou `bgc=` if `color=` não especificado).

Um material é considerado uma textura se incluir uma `src=` que especifica uma imagem e se ela ocorrer em um MSS diferente de decalque ou borda de parede.

Ao renderizar, a textura é alinhada com o objeto ao combinar a variável `anchor=` ponto do material de textura com o ponto de origem da textura do objeto (tal como criado na vinheta).
