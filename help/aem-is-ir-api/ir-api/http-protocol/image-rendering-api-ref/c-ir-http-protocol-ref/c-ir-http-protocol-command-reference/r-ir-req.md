...
<table id="table_A30296D29B5D43F1B5383A887252C6B4"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Real </p> </td> 
   <td colname="col3"> <p>Resolução máxima do objeto em <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> unidades de resolução do material </a> (normalmente pixels/polegada). </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> vinheta.res.avg </span> </p> </td> 
   <td colname="col2"> <p>Real </p> </td> 
   <td colname="col3"> <p>Resolução média de objetos em <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> unidades de resolução de material </a> (tipicamente pixels/inc <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local"> resolução de material </a>h). </p> </td> 
  </tr> 
  <tr> 
...
Os dados do mapa não são dimensionados para `wid=` ou modificados `hei=` de outra forma. O tipo MIME da resposta é `<text/xml>`.

Os dados de resposta consistem em um `<map>` elemento que contém um conjunto de `<area>` elementos, semelhante à `<AREA>` tag HTML.

Cada `<area>` elemento inclui os atributos padrão `type=` e `coord=` , bem como um `name=` atributo, especificando o nome do grupo de vinheta ou o caminho do nome. Vários `<area>` elementos com o mesmo nome estarão presentes se as máscaras do grupo de objetos correspondente tiverem regiões descontínuas.

Além dos atributos padrão, as vinhetas podem definir atributos adicionais se tiverem sido criadas. Tais atributos personalizados são definidos como atributos de grupo de objetos. Os nomes dos atributos personalizados devem começar com `map`. a incluir nos `<area>` elementos. Por exemplo, se os atributos do grupo incluírem `map.href=http://www.scene7.com`, o `<area>` elemento correspondente incluirá `href="http://www.scene7.com"`.

Um documento XML com um elemento vazio será retornado se a vinheta não incluir dados de mapa. `<map>`

`object`

Executa os comandos especificados e retorna a imagem renderizada mascarada pela seleção do objeto residual (o grupo ou objeto selecionado com o último `sel=` comando ou `obj=` comando na solicitação). Normalmente usado em conjunto com um formato de imagem que suporta alfa (consulte [fmt=](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c)). Se um formato de imagem for usado que não suporta alfa, as áreas fora da máscara serão pretas.

`props`

Executa os comandos especificados e retorna propriedades de vinheta e propriedades de grupo ou objeto, em vez da imagem renderizada. Consulte [Propriedades](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-response-data/c-ir-properties.md#concept-e99f1a373eae4af9b41842ca0088ad3a) para obter uma descrição da sintaxe de resposta e do tipo MIME de resposta. A seleção padrão se aplica a menos que `obj=` ou `sel=` seja especificada também (consulte [ `obj=` ](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-obj.md#reference-31e7dac7931b4e0eb3c7589f120a1e6a)).
...
<table id="table_F3ECF0584F6247A2A75C1CAFE1C57A4E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> Qualidade JPEG e sinalizador de croma (consulte <span class="codeph"> qlt <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-qlt.md#reference-27b91c226eb241d0a14a29af3b3afdbd" type="reference" format="dita" scope="local"> = </a> </span>) </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> image.type </span> </p> </td> 
   <td> <p> String </p> </td> 
   <td> <p> Tipo de MIME para a imagem de resposta (consulte <span class="codeph"> fmt <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c" type="reference" format="dita" scope="local"> = </a> </span>). </p> </td> 
  </tr> 
  <tr> 
...
Retorna o conteúdo de `vignette::UserData`. O servidor substituirá todas as ocorrências de `'??'` in `vignette::UserData` por terminadores de linha ( `<cr><lf>`). A resposta é formatada como dados de texto com o tipo MIME de resposta definido como &lt;text/plain>.

Se o objeto especificado no caminho do URL não for resolvido para uma entrada válida do mapa de vinheta, ou se o objeto estiver vazio, a resposta conterá apenas um terminador de linha ( `vignette::UserData` `CR/LF`).
