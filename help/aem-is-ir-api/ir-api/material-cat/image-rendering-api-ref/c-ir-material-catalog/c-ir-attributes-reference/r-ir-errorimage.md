...
Sequência de caracteres de texto. Se especificado, deve ser um **`ImageServing::catalog::id`** valor, um caminho relativo (para **`ImageServing::attribute::RootPath`** ou **`ImageRendering::attribute::RootPath`**) ou um caminho absoluto para um arquivo de imagem acessível pelo Servidor de imagens.
