...
Coleções de regras de pré-processamento (*conjuntos de regras*) pode ser anexado a catálogos de materiais ou ao catálogo padrão. As regras no catálogo padrão se aplicam somente se a solicitação não anexar um catálogo de materiais específico.
...
O `<?xml>`, `<!DOCTYPE>` e `<ruleset>` os elementos são sempre necessários em um arquivo XML de conjunto de regras válido, mesmo se nenhuma regra real estiver definida.
...
O comportamento padrão pode ser modificado com a variável `OnMatch` do `<rule>` elementos. `OnMatch` pode ser definido como `break` (padrão), `continue`ou `error.`
...
`<rule>` elementos podem definir opcionalmente atributos que substituem os atributos de catálogo correspondentes quando a regra for correspondida com sucesso e `OnMatch="break"` está definida. Nenhum atributo será aplicado se `OnMatch="continue"` está definida. Consulte a descrição de `<rule>` para uma lista de atributos que podem ser controlados com regras.
...
[pacote java.util.regex](https://www2.cs.duke.edu/csed/java/jdk1.4.2/docs/api/) descreve a implementação de expressão regular específica usada pelo Serviço de imagem.
