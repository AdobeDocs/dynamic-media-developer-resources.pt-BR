...
A pasta para a qual a função [!DNL Platform Server] grava arquivos de log. Pode ser um caminho absoluto ou relativo a *`install_folder`*. O padrão é [!DNL  *`install_folder`*/logs].
...
Prefixo de nome do arquivo no qual os dados de log de acesso são gravados. A data e o sufixo do arquivo ( [!DNL  *`yyyy`*-*`mm`*-*`dd`*.log]) são anexadas à string especificada. O nome do arquivo de log de acesso deve ser diferente do arquivo de log de rastreamento. O padrão é &quot; `access-`&quot;.

## TC::padrão - Padrão de Log de Acesso {#section-22775ea85cee444d8a7d7336a3b1feef}

Especifica o padrão de dados para [!DNL Platform Server] acessar registros de log. A string de padrão especifica variáveis que são substituídas por seus valores correspondentes. Todos os outros caracteres na cadeia de caracteres do padrão são transferidos literalmente para o registro de log.

Para usar o utilitário de aquecimento de cache, os espaços devem ser usados como separadores de campo. O [!DNL Platform Server] substitui todos os espaços e caracteres &#39;%&#39; em valores de campo por `%20` e `%25`, respectivamente.
...
<table id="table_7A07AFF34B5040A5B30F5735CFE9428F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>data e hora, formatada como <span class="codeph"> <span class="varname"> aaaa </span>- <span class="varname"> MM </span>- <span class="varname"> dd </span> <span class="varname"> HH </span>: <span class="varname"> mm </span>: <span class="varname"> ss </span>. <span class="varname"> SSS </span> offset </span> </p> <p> ( <span class="varname"> SSS </span> são msec, <span class="varname"> offset </span> é o desvio de tempo GMT); o valor de hora é capturado quando a resposta é enviada ao cliente. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>[!DNL Platform Server] palavra-chave do gerenciamento de cache: <span class="codeph"> { REUSED | CRIADO | ATUALIZADO | REMOTO | REMOTE_CREATED | REMOTE_UPDATE | REMOTE_CACHE | VALIDADO | IGNORADO | NÃO DEFINIDO } </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>O <span class="codeph"> tag </span> valor do cabeçalho de resposta (assinatura MD5 dos dados de resposta). </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Endereço IP do servidor peer no cluster de cache que forneceu a entrada do cache ou '-' se <span class="codeph"> CacheUse </span> não é <span class="codeph"> REMOTE_CREATED </span> nor <span class="codeph"> REMOTE_UPDATED </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>O tempo que leva [!DNL Platform Server] para enviar resposta após gravar dados no fluxo de saída. </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{Size}r </span> </p> </td> 
   <td> <p>Like <span class="codeph"> %B </span>, mas inclui valores para respostas 304 (não modificadas). </p> </td> 
  </tr> 
  <tr> 
