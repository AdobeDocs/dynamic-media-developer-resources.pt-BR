...
O servidor retorna ao cliente a representação que corresponde à localidade especificada com `locale=`, evitando assim a localização do lado do cliente e permitindo que os aplicativos alternem localidades simplesmente enviando o `locale=` valor apropriado com as solicitações de texto IS.
...
<table id="table_83344EFCB5B5418184E0A0B43D0B23F7"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>O valor de qualquer <span class="codeph"> público alvo.*.rótulo </span> e <span class="codeph"> público alvo.*.userdata </span> . </p> </td> 
  </tr> 
  <tr> 
...
Os *`string`* elementos habilitados para Localização no catálogo de imagens consistem em uma ou mais strings localizadas, cada uma precedida por um token de localização.
...
<table id="simpletable_0A687FA72C4C4C1AAFFCB43143C1AB3B"> 
 ...
 <tr class="strow"> 
  ...
  <td class="stentry"> <p>ID de localidade interna da <span class="varname"> localizedString </span> após este <span class="varname"> localizationToken </span>. </p> </td> 
 </tr> 
 <tr class="strow"> 
...
Valores vazios indicam que o *`locId`* *`defaultString`* deve ser retornado, se disponível.

Consulte a descrição do `attribute::LocaleStrMap` para obter detalhes.

## O processo de tradução {#section-a2a8a3e5850f4f7c9d2318267afe98a2}

Dado o mapa de tradução de exemplo acima e a solicitação `/is/image/myCat/myItem?req=&locale=nl`, o servidor procura primeiro &quot; `nl`&quot; no mapa de localidade. A entrada correspondente `nl,N` indica que para cada *`stringElement`*, o *`localizedString`* marcado com `^loc=N^` deve ser retornado. Se isso não *`localizationToken`* estiver presente no *`stringElement`*, um valor vazio será retornado.

Digamos `catalog::UserData` que `myCat/myItem` contenha o seguinte (quebras de linha inseridas para maior clareza):
...
No exemplo acima, `attribute::LocaleStrMap` tem uma entrada com um *`locale`* valor vazio. O servidor usa essa entrada para manipular todos os `locale=` valores que não são explicitamente especificados de outra forma no mapa de conversão.

O mapa de tradução de exemplo especifica que, nesse caso, o *`defaultString`* deve ser retornado se disponível. Assim, o seguinte seria devolvido se este mapa de tradução fosse aplicado ao pedido `/is/image/myCat/myItem?req=&locale=ja`:
...
Vários *`locId`* valores podem ser associados a cada um *`locale`* no mapa de conversão. Isso permite suportar variações específicas do país ou de região (por exemplo, inglês dos EUA vs inglês do Reino Unido) para seleção *`stringElements`* enquanto lida com a maioria dos conteúdos com localidades de base comuns (por exemplo, inglês internacional).

Para o nosso exemplo, queremos adicionar suporte para inglês específico dos EUA ( ` *`locId`* EUS`) e inglês específico do Reino Unido ( ` *`locId`* EUK`), para suportar a ortografia alternativa ocasional. Se a EUK ou a EUS não existissem, voltaríamos para a E. Do mesmo modo, as variantes alemãs específicas da Áustria ( `DAT`) poderiam ser disponibilizadas sempre que necessário, retornando o alemão comum *`localizedStrings`* (marcado com `D`) durante a maior parte do tempo.

`attribute::LocaleStrMap` se pareceria com isto:

`en,E|en_us,EUS,E|en_uk,EUK,E|de,D|de_at,DAT,D|de_de,D`

A tabela a seguir descreve a saída de algumas combinações representativas *`stringElement`* e *`locale`* combinações:
...
<table id="table_A6B67587C5F44B5E9CD0E7ED29A81198"> 
 ...
 <tbody> 
  ...
  <tr valign="top"> 
   <td> <p> <span class="codeph"> ^ loc=en^English^loc=USE^US-English^loc=D^German^loc=DDE^Deutsch </span> </p> <p> Observe que, neste exemplo, o <span class="varname"> locId </span> DDE não existe no <span class="codeph"> atributo::LocaleStrMap </span>, e, portanto, a subsequência de caracteres associada a esse <span class="varname"> locId </span> nunca é retornada. </p> </td> 
