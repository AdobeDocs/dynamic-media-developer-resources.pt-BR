...
`textPs=` O suporta recursos estendidos, como justificação, fluxo de texto para regiões não retangulares definidas com `textFlowPath=` e/ou `textFlowXPath=`e renderização de texto ao longo de caminhos arbitrários definidos com `textPath=`.

## Propriedades {#section-a289dc26b6534b41998b1e241d5f2f92}

Atributo de camada. Aplicável a `layer=0` se `layer=comp`. Mutualmente exclusivo com `src=` e `text=` na mesma camada. A última ocorrência de `text=`, `textPs=`, e `src=` prevalece e determina se é uma imagem ou uma camada de texto. Ignorado pelas camadas de efeito.
