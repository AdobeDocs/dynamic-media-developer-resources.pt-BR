...
`Auto` usa as configurações de digitalização computadas pela biblioteca JPEG independente e dependem do modelo de cores. Os valores de `3`, `4`, `5` correspondem à configuração Digitalização encontrada no Adobe Photoshop quando você salva um arquivo JPEG como pjpeg (JPEG progressivo).

Se não `pscan` estiver definido, o padrão será `auto`.
