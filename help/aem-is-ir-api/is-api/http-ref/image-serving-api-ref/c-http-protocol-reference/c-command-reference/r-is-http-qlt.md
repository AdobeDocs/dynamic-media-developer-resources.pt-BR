...
Atributo de solicitação. Aplica-se independentemente da configuração de camada atual. Ignorado se o formato de arquivo de imagem de saída não oferecer suporte à codificação JPEG. Consulte a descrição do para obter informações `fmt=` sobre quais formatos de imagem de saída são compatíveis `qlt=`.
