...
Os elementos `<?xml>` e `<ruleset>` sempre são necessários em um arquivo XML de conjunto de regras válido, mesmo se nenhuma regra real for definida.
...
Uma cópia do [!DNL RuleSet.xsd] é fornecida na pasta do catálogo e deve ser usada para validar um arquivo de conjunto de regras antes de registrá-lo no [!DNL catalog.ini] arquivo. Observe que o Serviço de imagem usa uma cópia interna do [!DNL RuleSet.xsd] para validação.
...
O comportamento padrão pode ser modificado com o `OnMatch` atributo do `<rule>` elemento. `OnMatch` pode ser definido como `break` (padrão), `continue`ou `error`.
...
`<rule>` como opção, os elementos podem definir atributos que substituem os atributos do catálogo correspondente quando a regra for correspondida com êxito. Se várias regras correspondentes definirem o mesmo atributo, o último prevalecerá. Consulte a descrição do ` [<rule>](../../../../../is-api/image-catalog/image-serving-api-ref/c-image-catalog-reference/c-rule-set-reference/r-rule-rule.md#reference-af76c0e2b8be48dabb52b71fe7e51ee9)` elemento para obter uma lista de atributos que podem ser controlados com regras.
...
[ [!DNL package java.util.regex] ](https://www2.cs.duke.edu/csed/java/jdk1.4.2/docs/api/) descreve a implementação de expressão regular específica usada pelo Serviço de Imagens.
