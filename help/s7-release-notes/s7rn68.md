...
description: As notas de versão mais recentes descrevem os recursos, aprimoramentos e correções mais recentes do Adobe Scene7 Publishing System 6.8, parte da solução Adobe Experience Manager na Adobe Experience Cloud.
...
As notas de versão mais recentes descrevem os recursos, aprimoramentos e correções mais recentes do Adobe Scene7 Publishing System 6.8-parte da solução Adobe Experience Manager na Adobe Experience Cloud.
...
* [Notas de versão para Visualizadores do Dynamic Media Image Serving 5.0.1](s7rn68.md#section-3fc1a827bcc84c0488229d310086ad8e)
...
* Suporte adicionado para web design responsivo — Servidor de imagens responsivo com base em pontos de interrupção e visualizadores responsivos do HTML5 (Conjuntos de imagens, Vídeo, Mídia mista, eCatalog, Spinsets, Swatchsets, Zoom) que redimensionam dinamicamente com base no layout da página.
...
* O Adobe Analytics vídeo relata a integração do Visualizador de vídeo HTML5 com o Visualizador de mídia mista.
* Rastreamento de eventos de vários visualizadores do HTML5 na mesma página.
* Adição de `SetIndicator` ao Visualizador de zoom do HTML5 e ao Visualizador de mix de mídia do HTML5 para representar visualmente amostras em dispositivos móveis.
* Adição da API `getComponent` para permitir acesso externo aos componentes do visualizador do HTML5.
...
* Modelos do JavaScript no HTML5 eCatalog Viewer para vinculação de mapas de imagem a páginas externas.
...
* Manipulador do Analytics para visualizadores do HTML5 para suporte a rastreamento de terceiros.
...
* Incluir na lista de bloqueios Para impedir que o seja executado, converta o email de senha esquecida do HTML em texto sem formatação.
* Eliminação da função padrão.
* Além da capacidade de carregar conteúdo para o Sistema de publicação do Scene7 por meio de upload de diretório de servidor (ftp) e URLs de upload, agora também é possível carregar conteúdo usando HTTP POST. Consulte a [API do Sistema de Produção de Imagens](/help/aem-ips-api/c-overview.md) para obter mais informações sobre o uso de HTTP POST.
...
* Visualizadores do HTML5 não carregam devido a UserAgent não reconhecido.
...
* Visualizadores do HTML5 impedem a impressão no navegador Firefox.
...
* O autoResize não deve estar no código Copiar URL/Incorporação do visualizador do HTML Flyout.
* Nenhum modificador videoserverurl no URL de cópia do visualizador de vídeo do HTML5.
* O parâmetro `contenturl` é da URL de visualização do Visualizador do HTML5 do servidor de Publicação.

**Imagens**

* Não foi possível adicionar a visualização ajustada como imagem padrão na configuração de publicação da imagem.
...
* O plug-in Flash e o Sistema de publicação do Scene7 falharam porque muitas pastas não tinham memória Flash suficiente.
* Ao remover o valor do campo definido pelo usuário Data/Hora, o Sistema de publicação do Scene7 não envia uma solicitação para o IPS (Sistema de publicação de imagem).
* Se as opções de codificação de vídeo digital estiverem ativadas, o reprocessamento de um arquivo do PDF ou do PSD exibirá a mensagem de aviso Codificação de vídeo adaptável.
...
* Solicitação de sistema de publicação do Scene7 s7info por meio de HTTP ao carregar conteúdo s7info.
* O URL no email Esqueci a senha é HTTP, mesmo que tenha vindo do Sistema de publicação SSL Scene7.
* Caracteres especiais eram codificados em um email formatado com HTML &quot;Esqueceu a senha&quot;, fazendo com que a senha não fosse exibida corretamente.
* As informações da conta FTP do Scene7 estão ausentes no email de boas-vindas de uma conta de avaliação
...
* &quot;Email Communications&quot; e servlet SendMailSimple.jsp do Sistema de publicação do Scene7.
...
* Botão Fechar exibido no modo de tela cheia em visualizadores do HTML5.

## Notas de versão do Dynamic Media Image Serving Viewers 5.0.1 {#section-3fc1a827bcc84c0488229d310086ad8e}

Ver [https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html)

## Novos recursos e correções de bugs no Scene7 SDK 2.7.1 {#section-b82b4f546ce44a769e94fb1493511035}
...
* Correção: Protocolo HTTPS gerando aviso de segurança no IE8 usando a reprodução Flash.
* Correção: o vídeo não é dimensionado corretamente no modo de tela cheia no Safari 5 usando a reprodução Flash.
...
* Adição de suporte responsivo ao código incorporado do HTML5 Viewers.
* O código incorporado para visualizadores do HTML5 passa os parâmetros de configuração como um único objeto JSON.
