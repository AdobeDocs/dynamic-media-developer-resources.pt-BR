---
description: As notas de versão mais recentes que descrevem os recursos, os aprimoramentos e as correções mais recentes do Adobe Scene7 Publishing System 6.8-parte da solução Adobe Experience Manager na Adobe Marketing Cloud.
seo-description: As notas de versão mais recentes que descrevem os recursos, os aprimoramentos e as correções mais recentes do Adobe Scene7 Publishing System 6.8-parte da solução Adobe Experience Manager na Adobe Marketing Cloud.
...
As notas de versão mais recentes que descrevem os recursos, os aprimoramentos e as correções mais recentes do Adobe Scene7 Publishing System 6.8-parte da solução Adobe Experience Manager na Adobe Marketing Cloud.

## Notas de versão do Scene7 Publishing System 6.8 (maio de 2014) {#topic-4517aab837ea4d6d8204425958860e5a}

As notas de versão mais recentes que descrevem os recursos, melhorias e correções mais recentes para [!DNL Adobe Scene7 Publishing System] 6,8 partes da solução Adobe Experience Manager no [!DNL Adobe Marketing Cloud].
...
* [Notas de versão do Scene7 Image Serving Viewers 5.0.1](s7rn68.md#section-3fc1a827bcc84c0488229d310086ad8e)
...
* Integração do Relatório de vídeo do Adobe Analytics.
...
* O vídeo do Adobe Analytics relata a integração do Visualizador de vídeo HTML5 e do Visualizador de mídia mista.
...
* Adição `getComponent` da API para permitir acesso externo aos componentes do visualizador HTML5.
...
Consulte também Guia [de referência do](/help/aem-viewers-ref/t-installing-the-viewers.md)Scene7 Viewers.
...
* Adicionadas operações de mascaramento com nitidez para Reprocessar trabalhos de ativos.
...
* Converter e-mail de senha esquecida de HTML em texto sem formatação para evitar a listagem de bloqueios.
* Eliminação da função padrão.
* Além da capacidade de fazer upload de conteúdo para o Scene7 Publishing System por meio do upload de diretório do servidor (ftp) e de URLs, agora você também pode fazer upload de conteúdo usando HTTP POST. Consulte a API [do sistema de produção de](/help/aem-ips-api/c-overview.md) imagens para obter mais informações sobre o uso do HTTP POST.
...
* O Visualizador de vídeo HTML5 impede o acesso ao plug-in Flash de desbloqueio.
...
* Ao inicializar as configurações de empresa, o Scene7 Publishing System envia a opção duplicado `<defaultValue>` na API `<createMetadataField>`.

* O plug-in Flash e o Scene7 Publishing System falharam devido a um grande número de pastas que não têm memória Flash suficiente.
...
* O Scene7 Publishing System solicita s7info por meio de HTTP ao carregar o conteúdo s7info.
* O URL no e-mail Esqueci a senha é HTTP, mesmo se ele veio do SSL Scene7 Publishing System.
...
* A atualização de maio de 2014 do Adobe Illustrator CC (Creative Cloud) e do Adobe InDesign CC deve introduzir incompatibilidades com plug-ins que criam o Scene7 FXG. Essas incompatibilidades afetam os usuários da solução Scene7 Web-to-Print. Se você usar o Scene7 Web-to-Print, não atualize o Illustrator e o InDesign para esta atualização da Creative Cloud. Em vez disso, continue usando a versão anterior a maio de 2014. A Adobe Systems espera a restauração da compatibilidade da criação Scene7 FXG com as versões mais recentes do Illustrator e do InDesign no período do último trimestre de 2014.
...
## Recursos não compatíveis {#section-7b04b6fdb8d1473880ccaf84211254ad}

* A separação de sílabas de texto será dessuportada na versão de outono do Scene7 6.9. Os dicionários de separação de sílabas definem como as palavras podem ser divididas e separadas para suportar o fluxo de texto ideal.
* A partir de 31 de janeiro de 2014, o Scene7 encerrou oficialmente o suporte para a plataforma do visualizador DHTML.
...
* Os banners dinâmicos (Flash) estão sendo descontinuados nesta versão. A adoção deste recurso tem sido baixa à medida que outras tecnologias surgiram para criar, gerenciar e fornecer banners personalizados para o site. A funcionalidade de banner dinâmico (Flash) do Scene7 continuará funcionando no lado do servidor, mas não será mais suportada.
* Lembrete: O Servidor de Metadados e o uso de APIs associadas foram oficialmente dessuportados a partir da versão anterior do Scene7.
* As extensões do Scene7 Creative Suite (painéis Flash) serão descontinuadas no final de 2014. O desenvolvimento adicional de workflows de ingestão de conteúdo está focado no Adobe Experience Manager Dynamic Media e na Creative Cloud. No Scene7, os recursos de upload e exportação no Scene7 Publishing System permanecerão e poderão ser usados.
...
* Seção de conta FTP Scene7 no modelo de email de boas-vindas para o usuário do Portal de mídia.
...
## Notas de versão do Scene7 Image Serving Viewers 5.0.1 {#section-3fc1a827bcc84c0488229d310086ad8e}

Consulte [https://marketing.adobe.com/resources/help/en_US/s7/viewers_ref/c_rn_04_29_14.html](https://marketing.adobe.com/resources/help/en_US/s7/viewers_ref/c_rn_04_29_14.html)
...
* Fixo: `selectSwatch` não é acionado quando incorporado na `initViewer()` função.
...
* Fixo: O ícone Compartilhamento em redes sociais não desaparece com o `controlbar` botão de opção em Zoom.
...
* Adicionado suporte para `initialBitRate` modificador para selecionar a taxa de bits usada para reprodução inicial na área de trabalho.
...
* Fixo: Protocolo HTTPS gerando aviso de segurança no IE8 usando a reprodução Flash.
* Fixo: O vídeo não é dimensionado corretamente no modo de tela cheia no Safari 5 usando a reprodução Flash.
