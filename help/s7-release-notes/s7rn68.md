...
As notas de versão mais recentes que descrevem os recursos, aprimoramentos e correções mais recentes do [!DNL Adobe Scene7 Publishing System] 6.8-parte da solução Adobe Experience Manager no [!DNL Adobe Experience Cloud].
...
* [Criação de imagens](s7rn68.md#section-113f27ea04cd4998a324bba71519c93c)
...
* [Recursos não são mais compatíveis](s7rn68.md#section-7b04b6fdb8d1473880ccaf84211254ad)
* [Notas de versão do Dynamic Media Image Serving Viewers 5.0.1](s7rn68.md#section-3fc1a827bcc84c0488229d310086ad8e)
* [Novos recursos e correções de erros no SDK 2.7.1 do Scene7](s7rn68.md#section-b82b4f546ce44a769e94fb1493511035)
...
* Adicionado `SetIndicator` para o Visualizador de zoom HTML5 e o Visualizador de mídia mista HTML5 para representar visualmente amostras em dispositivos móveis.
* Adicionado `getComponent` API para permitir acesso externo a componentes do visualizador de HTML5.
...
* `stagesize` para o Visualizador de imagem suspensa HTML5.
* `initComplete` manipulador para visualizadores HTML5 para acionar funções após visualizadores.
...
* &quot;Espaço posterior&quot; não funciona; use `sa`, atualmente está `margb`.
* Ao inicializar as configurações da empresa, o Scene7 Publishing System envia a opção de duplicação `<defaultValue>` na API `<createMetadataField>`.
...
* Fixo: `selectSwatch` método não acionado quando incorporado no `initViewer()` função.
...
* Correção: o ícone Compartilhamento em redes sociais não desaparece com o `controlbar` em Zoom.
...
* Suporte adicionado para `initialBitRate` modificador para selecionar a taxa de bits usada para a reprodução inicial no ambiente de trabalho.
...
* Correção: artefatos exibidos após aplicação de zoom na imagem ou rotação no dispositivo Android™.
