...
* Adição do &quot;Código de inserção&quot; para simplificar a implantação de visualizadores.
   ...
   Usar o [!DNL Embed Code] O recurso permite revisar o código do visualizador do tipo de ativo selecionado. Você também pode copiar o código para a área de transferência para poder colá-lo em suas páginas da Web para implantação do visualizador. A edição do código não é permitida na caixa de diálogo Incorporar código. .

   A variável [!DNL Embed Code] O recurso está disponível ao abrir a variável [!DNL Preview: Viewer List] página ou a [!DNL URLs] painel no [!DNL Detail View] para qualquer um dos seguintes tipos de ativos:
...
* Exibição de metadados para exibir apenas campos que contêm valores
   ...
   Existe uma nova visualização de metadados predefinida chamada &quot;Todas as propriedades com valores&quot; que mostra somente as metapropriedades que contêm valores. As propriedades (do XMP, Campos definidos pelo usuário ou os novos Esquemas de metadados) sem valores são ocultas e não serão exibidas.
* As configurações de metadados agora estão agrupadas na interface do usuário de administração.

   Entrada **[!UICONTROL Setup > Application Setup]**, as páginas de configuração relacionadas aos metadados agora são agrupadas em uma seção de nível superior chamada [!DNL Metadata]. Aqui você encontra as páginas de configuração para Exibição de metadados, Predefinições de metadados, Esquema de metadados e Campos definidos pelo usuário.
