---
description: As notas de versão mais recentes que descrevem os novos recursos, melhorias e correções do Adobe Scene7 Publishing System 6.10-parte da solução Adobe Experience Manager na Adobe Marketing Cloud.
seo-description: As notas de versão mais recentes que descrevem os novos recursos, melhorias e correções do Adobe Scene7 Publishing System 6.10-parte da solução Adobe Experience Manager na Adobe Marketing Cloud.
...
As notas de versão mais recentes que descrevem os novos recursos, melhorias e correções do Adobe Scene7 Publishing System 6.10-parte da solução Adobe Experience Manager na Adobe Marketing Cloud.

## Notas de versão do Scene7 Publishing System 6.10 (maio de 2015) {#topic-be8b44aea42643c496452726d4550226}

As notas de versão mais recentes que descrevem os novos recursos, melhorias e correções para [!DNL Adobe Scene7 Publishing System] 6,10 partes da [!DNL Adobe Experience Manager] solução no [!DNL Adobe Marketing Cloud].
...
* **Tipo** de pesquisa - Capacidade de adicionar propriedade de empresa para permitir o padrão do tipo de pesquisa pessoal **[!UICONTROL Contém]**.

   Em **[!UICONTROL Configuração]** > Grupo de configuração **** pessoal > cabeçalho **[!UICONTROL Navegador]** > lista suspensa Tipo **[!UICONTROL de pesquisa]** básica - **[!UICONTROL Contém]**.
...
<table id="table_69203DAA18F94A0F927BBA00634EBF65"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Pesquisa a partir do início da string e retorna os resultados da pesquisa mais rápido do que <span class="uicontrol"> Contém </span>. </p> <p>Substitui o padrão definido na lista suspensa <span class="uicontrol"> Configuração do aplicativo </span> &gt; <span class="uicontrol"> Grupo de configurações gerais </span> &gt; <span class="uicontrol"> Outras configurações, em Grupo do aplicativo </span> &gt; <span class="uicontrol"> Tipo de pesquisa básica padrão para novos usuários </span> pelo administrador. </p> </td> 
  </tr> 
 </tbody> 
</table>

Em Configuração **[!UICONTROL do]** aplicativo > Grupo de configurações **** gerais > **[!UICONTROL Outras configurações, em Grupo do aplicativo]** > Tipo de pesquisa básica **[!UICONTROL padrão para lista suspensa Novos usuários]** .
...
<table id="table_A48D69C0F26D43B58C4C890FBF908F8A"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol"> Inicia com </span> </p> </td> 
   <td colname="col2"> <p>Pesquisa a partir do início da string e retorna os resultados da pesquisa mais rápido do que <span class="uicontrol"> Contém </span>. </p> <p>Novos usuários podem optar por substituir esse padrão na Configuração <span class="uicontrol"> pessoal </span>. </p> </td> 
  </tr> 
 </tbody> 
</table>

* **Novas configurações adicionadas aos visualizadores** HTML5 Spin Set predefinidos - Em **[!UICONTROL configuração]** > Configuração **[!UICONTROL do]** aplicativo > **[!UICONTROL Visualizadores]** > Predefinições **[!UICONTROL do]** visualizador > `Universal_HTML5_SpinSet_dark` `Universal_HTML5_SpinSet_light`[!DNL Configure Viewer] ou, as seguintes adições foram feitas à respectiva página do visualizador:
...
<table id="table_65BA9F98D1D041109A44B1C130EF4EFA"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>A guia inclui uma configuração de qualidade de <span class="uicontrol"> impressão </span> que permite especificar o tamanho máximo da imagem que é enviada para a impressora. O padrão é <span class="codeph"> 1000 </span>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol"> Favoritos </span> </p> </td> 
   <td colname="col2"> <p>A guia inclui configurações para Tempo de <span class="uicontrol"> expiração </span> e Tamanho de miniatura <span class="uicontrol"> favoritos </span> (). </p> <p> 
...
* **Novas configurações adicionadas aos visualizadores** HTML5 Mixed Media Set predefinidos - Em **[!UICONTROL configuração]** > Configuração **[!UICONTROL do]** aplicativo > Predefinições **[!UICONTROL do]** visualizador > `Universal_HTML5_MixedMedia_dark` ou `Universal_HTML5_MixedMedia_light`, as seguintes adições foram feitas à respectiva [!DNL Configure Viewer] página do visualizador:
...
<table id="table_023203E3CC484F59A350C3C333EF817F"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol"> lista suspensa do modo de zoom </span> </p> </td> 
   <td colname="col2"> <p>Permite configurar o tipo de animação de zoom a ser usado. No painel esquerdo, clique em Recursos <span class="uicontrol"> do visualizador </span> para acessar as seguintes opções do modo de <span class="uicontrol"> zoom </span> na lista suspensa: </p> <p> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol"> Guia Flyout/Zoom em linha </span> </p> </td> 
   <td colname="col2"> <p> No painel esquerdo, clique em <span class="uicontrol"> Flyout/Inline Zoom </span> para acessar as seguintes opções disponíveis: </p> <p> 
     <ul id="ul_081A473EFAAC47DD8CF5260492A53C67"> 
      ...
      <li id="li_BB51273170054C3A8241F8F3B6856DA0"> <span class="uicontrol"> Duração da dica </span> (padrão: <span class="codeph"> 3 </span>), <span class="uicontrol"> Contagem de dicas </span> (padrão: <span class="codeph"> 1 </span>) e <span class="uicontrol"> Desvanecer </span> (padrão: <span class="codeph"> 0.3 </span>) - Essas três opções permitem que você especifique o comportamento de exibição para o texto opcional da dica. <p>Use a Duração da <span class="uicontrol"> dica </span> para especificar o número de segundos que o texto exibe antes de ser ocultado. Se você definir a duração como <span class="codeph"> -1 </span>, o texto da dica será exibido continuamente, mesmo se o usuário ativar o zoom. </p> <p>Use a Contagem de <span class="uicontrol"> dicas </span> para especificar o número de vezes que o texto é exibido ao exibir novas imagens no conjunto. Se você definir a contagem de dicas como <span class="codeph"> -1 </span>, o texto será exibido continuamente ao exibir qualquer imagem no conjunto. </p> <p> <span class="uicontrol"> Desvanecimento de dicas </span> permite especificar a duração de uma animação de desvanecimento que ocorre quando o texto é exibido ou desaparece. Se você definir o desaparecimento gradual da ponta como <span class="codeph"> 0 </span>, nenhuma transição de desaparecimento será usada. </p> </li> 
     </ul> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* O arquivo `<metadataArray>` estava vazio para o [!DNL .vtt] arquivo em resposta à API `getAssets`.

* Campo userdata ausente para orientação.
* A resposta de `getJobLogDetails` um trabalho atualizando um arquivo ACO não estava especificando os detalhes do trabalho corretamente.
* O arquivo mestre PSD foi registrado como publicado várias vezes por Servidor de imagens.
...
* Race a condição com restauração de pasta entre carregamento de diretório grande `deleteFolder` e de servidor.
...
* O processamento de vídeo não é mapeado `Iptc4xmpCore:Location` corretamente.
* A condição de corrida existia entre limpeza e carregamento.
* A publicação não usa, portanto, a publicação de preparo `assetFile` somente funciona para novos ativos.
* Ponteiro nulo lançado ao atualizar um conjunto de regras.

## Visualizadores {#section-833ab92c91c941d2bfdc27f233f582ad}

**Novos recursos, melhorias e correções de erros para os visualizadores Scene7 Image Serving 5.2.2**
...
* Fixo - os mapas de imagem do [eCatalog] ficam travados após o uso de modelos javascript para vinculação externa no Internet Explorer 9.
...
**Correção de erro para o Scene7 OnDemand 5.0.3**
...
* As configurações de publicação não exibiam perfis coloridos da empresa Ativos compartilhados do Scene7.
...
* Malformado `clientAddressFilter` causava uma exceção do servidor de Catálogo ao usar as configurações de Publicação de disponibilização de imagem de teste seguro.
...
* Remoção do botão do assistente [!DNL Adobe Marketing Cloud] de vinculação.
...
* Interface do usuário de configuração implementada para criar e atualizar campos de metadados de tipo de tag. O suporte para os Tipos **[!UICONTROL de campo de]** metadados no `fieldType` parâmetro inclui `SingleFixedTag` e `BooleanTag` apenas.
...
* O `Userdata` campo de valor foi adicionado novamente à pesquisa avançada.
* Ocorreu um erro ao adicionar um campo definido pelo usuário ( **[!UICONTROL Configuração]** > **Metadados** > Campo **definido pelo** usuário) do tipo `Boolean`, um valor de `true`e depois salvou-o.

## Recursos não compatíveis {#section-9295ea9313c84695b19bca0e0f01a690}

* **Aviso** de Fim da Vida Útil dos Visualizadores Flash *A partir de 31 de janeiro de 2017, o Adobe Scene7 Publishing System oferecerá suporte oficial ao fim da vida útil da plataforma do visualizador Flash.*
